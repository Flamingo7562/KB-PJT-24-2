...
`4.2.0`은 `dev`에서 구현 계약으로 사용한 `SPEC-151-01`, `SPEC-162-01`을 정식 명세에
통합한 하위 호환 Minor 릴리스입니다. 은행 코드 `004`, `011`의 화면 표시명을 각각
`KB국민은행`, `NH농협은행`으로 정합화하고, 사업장 생성과 고정 QR 최초 발급의 원자성,
조회·재발급, 5세그먼트 HMAC Token, 키 교체, 잠금과 오류 경계를 확정했습니다. QR 재발급은
현재 `Idempotency-Key`를 요구하지 않으며 후속 재시도 식별 방식은 열린 결정으로 유지합니다.
요청 Body, 은행 API 코드, DB 구조와 최소 호환 스키마는 변경하지 않습니다. 최소 호환선은
