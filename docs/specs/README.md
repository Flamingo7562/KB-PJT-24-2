...
`4.0.1`은 제품 동작과 최소 호환 스키마를 바꾸지 않고, 기능 개발에서 정식 SPEC과 관련
`draft` Patch를 함께 사용하는 경량 개발 계약 절차를 확정한 운영 Patch 릴리스입니다.
제품 계약은 `4.0.0`에서 확정한 Bearer 초대 생명주기, Work Case 닫힌 응답, 수락
Aggregate, 전자동의, 계약 파일과 접근 감사 계약을 그대로 유지합니다. 최소 호환선은
