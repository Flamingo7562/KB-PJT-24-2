...
| 승인일           | 2026-08-04                 |
| 소유자           | PM/Admin Master            |
| 최소 호환 스키마 | Flyway `202608041614` 이상 |
...
| Version | 승인일     | 요약                                                                                |
| ------- | ---------- | ----------------------------------------------------------------------------------- |
| `2.2.0` | 2026-08-04 | M3 Mock 계좌 프로비저닝·조회, 계좌·금액 오류 매핑(`403 FORBIDDEN` 통일), 거래 Page 표시 규칙, 멱등 Claim 상태 모델과 저장·재응답 범위를 확정한 `2.1.0` 위 호환 추가 릴리스 (이슈 #148) |
