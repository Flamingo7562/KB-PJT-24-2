...
`4.1.0`은 합성 Mock 계좌에서 사용할 수 있는 canonical 은행 코드와 표시 은행명을 기존
5개에서 20개로 확장한 하위 호환 Minor 릴리스입니다. 기존 5개 API 코드는 유지하고 은행
선택 화면의 표시명을 계약에 고정하며, DGB대구은행과 별도 선택지인 iM뱅크에는 프로젝트 전용
코드 `131`을 배정했습니다. 요청 Body, 입력 형식, DB 구조와 최소 호환 스키마는 변경하지
않습니다. 최소 호환선은
