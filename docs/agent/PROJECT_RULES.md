...
7. Treat `docs/spec-patches/` as the non-normative proposal and audit layer described in its `README.md`. A patch never changes the product contract by itself, and agents must not combine a patch with `docs/specs/` at implementation time to infer a newer contract.
...
10. Keep a patch proposal pull request isolated from application code, Flyway migrations, DDL or schema artifacts, and `docs/specs/`. An `accepted` patch permits parallel implementation work, but an implementation pull request must not merge until the Controller has published the patch through a canonical spec release; implementation pull requests record both `Spec Patch` and `Base Spec`.
11. In a Controller-owned canonical spec release, start from current `origin/dev`; recheck the patch base version and commit, target overlap, dependencies, and compatibility; then update all affected canonical documents, release metadata, changelog, any required compatibility baseline, `SPEC_LOCK.json`, and the patch's `applied` archive state atomically. Stale or conflicting patches require an explicit new revision or reapproval instead of automatic application.
12. Reverse a released contract only through a separate revert patch and a new Controller-owned canonical spec release. Never restore an older protected file directly, silently rewrite an accepted revision, or infer rollback authority from an implementation request.
