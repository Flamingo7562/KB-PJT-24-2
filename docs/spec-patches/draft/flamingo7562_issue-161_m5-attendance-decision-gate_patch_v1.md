...
CHECK_IN 종료 전까지 다음 자동 판정 주기에 READY가 될 수 있다.
...
Operation은 `ATTENDANCE_SCAN`이다. Fingerprint는 QR Token의 SHA-256 소문자 Hex,
...
| 좌표·정확도·측정 시각 형식 또는 범위 오류 | 400 | `VALIDATION_ERROR` |
...
이 동작은 현재 좌표, 근태, Settlement와 범용 Claim Column으로 구현할 수 있으므로 이 Patch는
Migration을 포함하지 않는다. CHECK_OUT_MISSING 후보 조회용 `(status, ends_at, id)` Index는
실제 MySQL `EXPLAIN`에서 필요성이 확인될 때 별도 관리자 승인 Migration으로 추가한다. M5는
수동 보정 감사 Column을 추가하지 않는다.
...
- [ ] 현재 Schema만으로 기능 검증을 마치고 Scheduler Index가 필요하면 별도 승인 Migration 근거로 `EXPLAIN`을 남긴다.
